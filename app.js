const UNOGS_API_URL = "https://unogs-unogs-v1.p.rapidapi.com/aaapi.cgi";
...
function getMovieListFromAPI() {
  ...
  function setHeader(xhr) {
    ...
    xhr.setRequestHeader(
      "x-rapidapi-key",
      "Yjhatosow1msh5mozm7dx40MtAu8p17ihADjsnoYWDUjeJpOH1"
    );
...
function requestAllMovieReviews() {
  ...
  appState.movies.forEach((movie) =>
    promises.push(getReviewsForMovieFromAPI(movie.imdbid))
  );
...
function setDetailsForMovie(details) {
  if (details.imdbID) {
    const movieIndex = appState.movies.findIndex(
...
function handleMovieClick() {
  $("#results").on("click", ".js-movie", function (event) {
    ...
    if (movie) {
      ...
      $("#results").html(
        '<h2 class="error">Movie details could not be found</h2>'
      );
